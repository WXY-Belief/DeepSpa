...
class CellPoseHead(BaseDecodeHead):
    ...
    def __init__(self, **kwargs):
        super(CellPoseHead, self).__init__(
    ...
    def forward_train(self,
        ...
        if return_last_feat:
            # bs ,3, h, w
            last_feat, seg_logit = self.forward(inputs, return_last_feat=return_last_feat)
        ...
        seg_logit = resize(
            input=seg_logit,
            size=gt_semantic_seg.shape[2:] if gt_semantic_seg is not None else dummy_gt_semantic_seg.shape[2:],
            mode='bilinear',
            align_corners=self.align_corners)
        if return_logits:
