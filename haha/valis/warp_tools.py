...
vipshome = r'D:\PycharmProjects\SPACEL_3d_align\vips-dev-8.10\bin'
...
def remove_invasive_displacements(bk_dxdy, M, src_shape_rc, out_shape_rc, inpaint_holes=False):
    ...
    if M is not None:
        affine_mask = warp_img(np.full(src_shape_rc, 255, dtype=np.uint8), M, out_shape_rc=out_shape_rc,
...
def warp_img(img, M=None, bk_dxdy=None, out_shape_rc=None,
    ...
    if transformation_dst_shape_rc is None:
        if bk_dxdy is not None:
            ...
            transformation_dst_shape_rc = np.ceil(np.max(warped_transformation_src_corners_xy[:, ::-1], axis=0)).astype(
                int)
    ...
    if do_rigid:
        ...
        tx_ty["tx"] = [tx]
        tx_ty["ty"] = [ty]
        tx_ty.to_csv("./tx_ty.csv", sep=",", header=True, index=False)
        ####################################
...
def warp_img_inv(img, M=None, fwd_dxdy=None, transformation_src_shape_rc=None, transformation_dst_shape_rc=None,
                 src_shape_rc=None, bk_dxdy=None, bg_color=None, interp_method="bicubic"):
...
def get_warp_map(M=None, dxdy=None, transformation_dst_shape_rc=None,
    ...
    if np.all(transformation_dst_shape_rc == dst_shape_rc):
        ...
        grid_r, grid_c = src_xy_pos[:, 1].reshape(transformation_dst_shape_rc), src_xy_pos[:, 0].reshape(
    ...
    if M is not None:
        ...
        xy_pos_in_src = [xy_pos_in_src[:, 0].reshape(transformation_dst_shape_rc),
...
def get_img_area(img_shape_rc, M=None):
    ...
    prev_area = 0.5 * np.abs(
        np.dot(prev_img_corners[:, 0], np.roll(prev_img_corners[:, 1], 1)) - np.dot(prev_img_corners[:, 1],
                                                                                    np.roll(prev_img_corners[:, 0], 1)))
    return prev_area
...
def get_warp_scaling_factors(transformation_src_shape_rc=None, transformation_dst_shape_rc=None, src_shape_rc=None,
                             dst_shape_rc=None, bk_dxdy=None, fwd_dxdy=None):
...
def _warp_pt_vips(xy, M=None, vips_bk_dxdy=None, vips_fwd_dxdy=None, src_sxy=None, dst_sxy=None, displacement_sxy=None,
                  displacement_shape_rc=None, pt_buffer=100):
...
def _warp_xy_vips(xy, M=None, transformation_src_shape_rc=None, transformation_dst_shape_rc=None,
    ...
    warped_xy = np.vstack([_warp_pt_vips(pt, M, vips_bk_dxdy=vips_bk_dxdy, vips_fwd_dxdy=vips_fwd_dxdy, src_sxy=src_sxy,
                                         dst_sxy=dst_sxy, displacement_sxy=displacement_sxy,
                                         displacement_shape_rc=displacement_shape_rc, pt_buffer=pt_buffer) for pt in
                           xy])
...
def warp_xy_inv(xy, M=None, transformation_src_shape_rc=None, transformation_dst_shape_rc=None, src_shape_rc=None,
                dst_shape_rc=None, bk_dxdy=None, fwd_dxdy=None):
...
def untangle(dxdy, n_grid_pts=50, penalty=10e-6, mask=None):
    ...
    if mask is not None:
        ...
        frozen_mask[0:frozen_mask.shape[0] - 1,
        frozen_mask.shape[1] - mesh.c_offset: frozen_mask.shape[1] - 1] = 0  # right
        frozen_mask[0:mesh.r_offset, 0:frozen_mask.shape[1] - 1] = 0  # top
        frozen_mask[frozen_mask.shape[0] - mesh.r_offset: frozen_mask.shape[0] - 1,
        0:frozen_mask.shape[1] - 1] = 0  # bottom
...
class QuadMesh(object):
    ...
    def __init__(self, dxdy, n_grid_pts=50):
        ...
        quads = [[r * nc + c, r * nc + c + 1, (r + 1) * nc + c + 1, (r + 1) * nc + c] for r in range(nr - 1) for c in
                 range(nc - 1)]
        self.quads = quads
    ...
    def show(self):
        ...
        for quad in self.quads:
            for e in range(4):
                ...
                line = ((off + self.x[i] * res / 2, off + self.x[i + self.nverts] * res / 2),
                        (off + self.x[j] * res / 2, off + self.x[j + self.nverts] * res / 2))
            ...
            for t in range(self.n_tri):
                ...
                for dim in range(2):
                    ...
                    for i in range(3):
                        ...
                        G[v + dim * self.n] += (self.ref_tri[t][i] @ np.transpose(dfda * (1. - theta) + dgda * theta)) * \
                                               self.area[t]
