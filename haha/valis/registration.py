...
class Valis(object):
        ...
        for f in full_path_list:
            if os.path.isdir(f):
                ...
                if is_round:
                    ...
                    matching_f = [ff for ff in full_path_list if
                                  re.search(dir_name, ff) is not None and os.path.split(ff)[1] != dir_name]
                    if len(matching_f) == 1:
    ...
    def process_imgs(self, processor_dict):
        ...
        for i, slide_obj in enumerate(tqdm.tqdm(self.slide_dict.values())):
            ...
            processor = processing_cls(image=slide_obj.image, src_f=slide_obj.src_f, level=level,
    ...
    def rigid_register(self):
        ...
        for slide_reg_obj in rigid_registrar.img_obj_list:
            ...
            slide_obj.rigid_reg_img_f = os.path.join(self.reg_dst_dir,
                                                     str.zfill(str(slide_obj.stack_idx),
                                                               n_digits) + "_" + slide_obj.name + ".png")
        ...
        for slide_name, slide_obj in self.slide_dict.items():
            ...
            warp_tools.save_img(slide_obj.rigid_reg_img_f, warped_img.astype(np.uint8),
    ...
    def _create_non_rigid_reg_mask_from_rigid_masks(self):
        ...
        temp_non_rigid_mask = 255 * filters.apply_hysteresis_threshold(combo_mask, 0.5, self.size - 0.5).astype(
            np.uint8)
    ...
    def prep_images_for_large_non_rigid_registration(self, max_img_dim,
        ...
        if not warp_full_img:
            all_max_dims = [np.any(np.max(slide_obj.slide_dimensions_wh, axis=1) >= max_img_dim) for slide_obj in
                            self.slide_dict.values()]
        ...
        for slide_obj in tqdm.tqdm(self.slide_dict.values()):
            ...
            src_img_shape_rc, src_M = warp_tools.get_src_img_shape_and_M(
            ...
            if max_img_dim is not None:
                closest_img_levels = np.where(np.max(slide_obj.slide_dimensions_wh, axis=1) < np.max(src_img_shape_rc))[
                    0]
            ...
            temp_slide_mask = slide_obj.warp_img(slide_obj.rigid_reg_mask, non_rigid=dxdy is not None, crop=False,
                                                 interp_method="nearest")
            ...
            if not use_tiler:
                ...
                processing_cls, processing_kwargs = processor_dict[slide_obj.name]
                processor = processing_cls(image=unprocessed_warped_img, src_f=slide_obj.src_f, level=closest_img_level,
                                           series=slide_obj.series)
                ...
                if self.norm_method is not None:
                    processed_img = preprocessing.norm_img_stats(img=processed_img,
                                                                 target_stats=self.target_processing_stats,
                                                                 mask=np_mask)
    ...
    def non_rigid_register(self, rigid_registrar, processor_dict):
        ...
        nr_on_scaled_img = self.max_processed_image_dim_px != self.max_non_rigid_registration_dim_px or \
                           (non_rigid_reg_mask is not None and np.any(
        ...
        for slide_name, slide_obj in self.slide_dict.items():
            ...
            if not using_tiler:
                ...
                cropped_bk_dxdy.cast("float").tiffsave(slide_obj._bk_dxdy_f, compression="lzw", lossless=True,
                                                       tile=True, bigtiff=True)
                cropped_fwd_dxdy.cast("float").tiffsave(slide_obj._fwd_dxdy_f, compression="lzw", lossless=True,
                                                        tile=True, bigtiff=True)

            slide_obj.nr_rigid_reg_img_f = os.path.join(self.non_rigid_dst_dir,
                                                        img_save_id + "_" + slide_obj.name + ".png")
    ...
    def register_micro(self, brightfield_processing_cls=DEFAULT_BRIGHTFIELD_CLASS,
        ...
        for slide_obj in self.slide_dict.values():
            ...
            slide_sxy = (np.array(out_shape) / np.array([vips_current_bk_dxdy.height, vips_current_bk_dxdy.width]))[
                        ::-1]
            ...
            if not write_dxdy:
                ...
                if not os.path.exists(slide_obj._bk_dxdy_f):
                    ...
                    cropped_bk_dxdy.cast("float").tiffsave(full_temp_dx_f, compression="lzw", lossless=True, tile=True,
                                                           bigtiff=True)
                ...
                if not os.path.exists(slide_obj._fwd_dxdy_f):
                    ...
                    cropped_fwd_dxdy.cast("float").tiffsave(full_temp_fwd_f, compression="lzw", lossless=True,
                                                            tile=True, bigtiff=True)
    ...
    def warp_and_merge_slides(self, dst_f=None, level=0, non_rigid=True,
        ...
        for f in src_f_list:
            ...
            if channel_name_dict is not None:
                ...
                if slide_channel_names is None:  # avoid exception
                    slide_channel_names = ["None"]
