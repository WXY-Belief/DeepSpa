...
def draw_cell_type_map(all_section_cell_center, all_section_cell_type, data_path, output_path, flag):
    star_time = time.time()
    ...
    color_list = generate_colors(len(cell_type_name))
    ...
    for item in all_section:
        ...
        for idx_1, item_1 in merge_data.iterrows():
            ...
            color = color_list[cell_type_name.index(item_1["cell_type"])]
        ...
        cv2.imwrite(os.path.join(save_path, "cell_type.PNG"), img)

        draw_legned(cell_type_name, color_list, save_path)
    print("Drawing image of cell type finished, runtime：", time.time() - star_time)
